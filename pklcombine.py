...
with open('./pkl/affiliation duplication run 1/GSauthorduplicate1.pkl', 'rb') as handle:
    data_store = pickle.load(handle)
df = pd.DataFrame(data=data_store[1], columns=data_store[0])

with open('./pkl/affiliation duplication run 1/GSauthorduplicate35.pkl', 'rb') as handle:
    data_store = pickle.load(handle)
df2 = pd.DataFrame(data=data_store[1], columns=data_store[0])

with open('./pkl/affiliation duplication run 1/GSauthorduplicate273.pkl', 'rb') as handle:
    data_store = pickle.load(handle)
df3 = pd.DataFrame(data=data_store[1], columns=data_store[0])

with open('./pkl/affiliation duplication run 1/GSauthorduplicate274.pkl', 'rb') as handle:
    data_store = pickle.load(handle)
df4 = pd.DataFrame(data=data_store[1], columns=data_store[0])

with open('./pkl/affiliation duplication run 1/GSauthorduplicate324.pkl', 'rb') as handle:
    data_store = pickle.load(handle)
df5 = pd.DataFrame(data=data_store[1], columns=data_store[0])

with open('./pkl/affiliation duplication run 1/GSauthorduplicate329.pkl', 'rb') as handle:
    data_store = pickle.load(handle)
...
with open('GSauthorduplicatewithaffiliationComplete.pkl', 'wb') as handle:
    pickle.dump([data_store_columns, personaldata.values], handle, protocol=pickle.HIGHEST_PROTOCOL)

write_excel = create_excel_file('./results/{}_results.xlsx'.format('GSauthorduplicatewithaffiliationComplete'))
